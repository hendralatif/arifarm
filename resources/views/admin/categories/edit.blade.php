...
<div class="max-w-2xl mx-auto space-y-6">
    <div>
        ...
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white">Edit Kategori</h1>
        <p class="text-sm text-slate-500">Perbarui rincian kategori atau ubah gambar banner.</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="space-y-1">
                ...
                <input type="text" id="name" name="name" required value="{{ old('name', $category->name) }}" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                @error('name')
            ...
            <div class="space-y-1">
                ...
                <textarea id="description" name="description" rows="4" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <span class="text-xs text-rose-500 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider block">Gambar Saat Ini</span>
                <div class="w-32 h-20 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
            ...
            <!-- New Image File -->
            <div class="space-y-1">
                <label for="image" class="text-xs font-bold text-slate-500 uppercase tracking-wider block">Ganti Gambar Banner</label>
