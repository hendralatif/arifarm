...
<div class="max-w-4xl mx-auto space-y-6">
    <div>
        ...
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white">Edit Rincian Kambing</h1>
        <p class="text-sm text-slate-500">Perbarui rincian fisik hewan ternak, status kesehatan, harga, atau ubah galeri foto.</p>
    </div>

    <div class="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm">
        <form action="{{ route('admin.goats.update', $goat->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="space-y-1">
                    ...
                    <select id="category_id" name="category_id" required class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $goat->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" id="name" name="name" required value="{{ old('name', $goat->name) }}" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    @error('name')
                        <span class="text-xs text-rose-500 block">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="space-y-1">
                    ...
                    <input type="number" id="price" name="price" required value="{{ old('price', (int)$goat->price) }}" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    @error('price')
                        <span class="text-xs text-rose-500 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" id="stock" name="stock" required value="{{ old('stock', $goat->stock) }}" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    @error('stock')
                        <span class="text-xs text-rose-500 block">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-5">
                <div class="space-y-1">
                    ...
                    <input type="number" step="0.01" id="weight_kg" name="weight_kg" required value="{{ old('weight_kg', $goat->weight_kg) }}" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    @error('weight_kg')
                        <span class="text-xs text-rose-500 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" id="age_months" name="age_months" required value="{{ old('age_months', $goat->age_months) }}" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    @error('age_months')
                        <span class="text-xs text-rose-500 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" id="breed" name="breed" required value="{{ old('breed', $goat->breed) }}" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    @error('breed')
                        <span class="text-xs text-rose-500 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <select id="gender" name="gender" required class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                        <option value="male" {{ old('gender', $goat->gender) == 'male' ? 'selected' : '' }}>Jantan</option>
                        <option value="female" {{ old('gender', $goat->gender) == 'female' ? 'selected' : '' }}>Betina</option>
                    </select>
                    @error('gender')
                        <span class="text-xs text-rose-500 block">{{ $message }}</span>
                    @enderror
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-5">
                <div class="space-y-1">
                    ...
                    <select id="acquisition_type" name="acquisition_type" required class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                        <option value="beli" {{ old('acquisition_type', $goat->acquisition_type) == 'beli' ? 'selected' : '' }}>Pembelian</option>
                        <option value="kelahiran" {{ old('acquisition_type', $goat->acquisition_type) == 'kelahiran' ? 'selected' : '' }}>Kelahiran</option>
                        <option value="lainnya" {{ old('acquisition_type', $goat->acquisition_type) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                    </select>
                ...
                <div class="space-y-1">
                    ...
                    <select id="health_status" name="health_status" required class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                        <option value="healthy" {{ old('health_status', $goat->health_status) == 'healthy' ? 'selected' : '' }}>Sehat Walafiat</option>
                        <option value="vaccine_completed" {{ old('health_status', $goat->health_status) == 'vaccine_completed' ? 'selected' : '' }}>Sertifikasi Vaksin Selesai</option>
                        <option value="under_observation" {{ old('health_status', $goat->health_status) == 'under_observation' ? 'selected' : '' }}>Dalam Pemantauan Medis</option>
                    </select>
                    @error('health_status')
                        <span class="text-xs text-rose-500 block">{{ $message }}</span>
                    @enderror
                ...
                <div class="space-y-1">
                    ...
                    <select id="vaccine_status" name="vaccine_status" required class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                        <option value="1" {{ old('vaccine_status', $goat->vaccine_status) == '1' ? 'selected' : '' }}>Sudah Vaksin</option>
                        <option value="0" {{ old('vaccine_status', $goat->vaccine_status) == '0' ? 'selected' : '' }}>Belum Vaksin</option>
                    </select>
                ...
                <div class="space-y-1">
                    ...
                    <select id="status" name="status" required class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                        <option value="available" {{ old('status', $goat->status) == 'available' ? 'selected' : '' }}>Tersedia (Ready)</option>
                        <option value="sold" {{ old('status', $goat->status) == 'sold' ? 'selected' : '' }}>Terjual (Sold)</option>
                        <option value="not_for_sale" {{ old('status', $goat->status) == 'not_for_sale' ? 'selected' : '' }}>Tidak Dijual</option>
                        <option value="mati" {{ old('status', $goat->status) == 'mati' ? 'selected' : '' }}>Mati (Deceased)</option>
                    </select>
                    @error('status')
                        <span class="text-xs text-rose-500 block">{{ $message }}</span>
                    @enderror
            ...
            <div id="purchase_price_wrapper" class="space-y-1 hidden">
                ...
                <input type="number" id="purchase_price" name="purchase_price" value="{{ old('purchase_price', $goat->purchase_price ? (int)$goat->purchase_price : '') }}" placeholder="Contoh: 2500000" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">
                <span class="text-[10px] text-slate-400 block">Informasi biaya ini akan langsung terhubung ke transaksi pengeluaran (pembelian hewan).</span>
            ...
            <div class="space-y-1">
                ...
                <textarea id="description" name="description" rows="5" class="w-full py-2.5 px-3.5 rounded-xl border-slate-200 dark:border-slate-800 dark:bg-slate-950 dark:text-white text-sm focus:border-emerald-500 focus:ring-emerald-500">{{ old('description', $goat->description) }}</textarea>
                @error('description')
                    <span class="text-xs text-rose-500 block">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider block">Foto Galeri Saat Ini</span>
                <div class="grid grid-cols-4 sm:grid-cols-6 gap-4">
                    @if(is_array($goat->images))
                        @foreach($goat->images as $img)
                            <div class="rounded-xl overflow-hidden aspect-square border border-slate-250 bg-slate-100 dark:bg-slate-950">
            ...
            <!-- Ganti Foto -->
            <div class="space-y-1">
                <label for="images" class="text-xs font-bold text-slate-500 uppercase tracking-wider block">Ganti Semua Foto (Multi-Upload)</label>
                ...
                <span class="text-[10px] text-slate-400 block mt-1">Mengunggah file baru akan menggantikan seluruh galeri foto saat ini.</span>
                @error('images')
                    <span class="text-xs text-rose-500 block">{{ $message }}</span>
                @enderror
            </div>
